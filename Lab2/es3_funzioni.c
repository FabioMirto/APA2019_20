...
void leggiFile(FILE *fp1, dat dati[], int num);
void trovaSequenze(dat dati[], test testo[], int num, int countWords);

int main() {
    int i = 0, j, p, l, m = 0, num = 0, countWords = 0, n;
    ...
    fscanf(fp1, "%d", &num);
   leggiFile(fp1, dati, num);

    while (!feof(fp2)) {
        fscanf(fp2, "%s", testo[m].parola);
        for (n = 0; n < strlen(testo[m].parola); n++) {
        ...
        m++;
          ...
          trovaSequenze(dati, testo, num, countWords)
