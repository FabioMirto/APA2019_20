...
static int checkDag(Edge *Sol, Edge **B_Sol, int *sol, Edge *edge, int *b_k, int V, int E, int pos, int *N);
...
Graph GRAPHcreateDAG(Graph G, Edge **archi, int N, int card, Edge *edge){
    ...
    for(i = 0; i < card; i++){
        printf("%s - %s peso: %d\n", STsearchByIndex(DAG->tab,archi[ind][i].v),
                STsearchByIndex(DAG->tab,archi[ind][i].w), archi[ind][i].wt);
    }
    for(j = 0; j < card; j++){
        for(i = 0; i < E; i++){
...
Edge ** GRAPHtoDAG(Graph G,int *l,int *N,Edge **edge){
    ...
    *edge = (Edge*) malloc(G->E *sizeof(Edge));
    ...
    checkDag(sol, b_sol, s, *edge, l, G->V, G->E, 0, N);
    (*N)++;
    free(sol);
    free(s);
    return b_sol;
...
static int checkDag(Edge* Sol, Edge** B_Sol, int *sol, Edge *edge, int *b_k, int V, int E, int pos, int *N){
    ...
    sol[pos] = 0;
    checkDag(Sol, B_Sol, sol, edge, b_k, V, E, pos+1, N);
    sol[pos] = 1;
    checkDag(Sol, B_Sol, sol, edge, b_k, V, E, pos+1, N);
    return 1;
}
...
int GRAPHdfs(Graph G, int id) {
    ...
    for (v = 0; v < G->V; v++) {
        if (pre[v] == -1) {
            dfsR(G, EDGEcreate(v, v, 1), &time, pre, post, st, &flag);
            if (!flag) {
                free(pre);
                free(post);
                free(st);
