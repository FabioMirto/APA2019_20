...
void pgList_print(FILE *fp, pgList_t pgList, invArray_t invArray){
    ...
    for(x = pgList; x != NULL; x = x->next){
        printf("%s %s %s %d %d %d %d %d %d\n", x->pg.cod, x->pg.nome, x->pg.classe,
               x->pg.b_stat.hp, x->pg.b_stat.mp, x->pg.b_stat.atk, x->pg.b_stat.def,
               x->pg.b_stat.mag, x->pg.b_stat.spr);
    }
}
...
pg_t *pgList_searchByCode(pgList_t pgList, char* cod){
    pgList_t x;
    printf("Digita un codice per ritornare il puntatore\n");
    scanf("%s", cod);
    for(x = pgList; x != NULL; x = x->next){
